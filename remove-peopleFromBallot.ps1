...
python -m stv.stv -b $ballotsPath -c $constituenciesPath -q $quota -s $seats | out-file "$outpath.csv"
Get-Content "$outpath.csv"

& "$psscriptroot/format-zeusOutput.ps1" -file "$outpath.csv" -title "$title" | pandoc -s -o "$outpath.pdf"
#>